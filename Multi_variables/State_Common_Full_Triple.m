...
Num_Determining_States_In_Each_Combination = zeros(Num_var^2,size(States,2));
...
for i=1:Num_State
    ...
    for dbl_indx=find(Interact(i,:))
        for tpl_indx = find(Interact(dbl_indx,:))
            cnt = cnt+1;
            index = [i find(Interact(i,:)) dbl_indx find(Interact(dbl_indx,:)) tpl_indx find(Interact(tpl_indx,:))];
