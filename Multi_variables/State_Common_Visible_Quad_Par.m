...
clc
%clear States gState PIP Num_common_States index Mean_Common_State index;
clear all
...
for i=1:Num_State
    ...
    for dbl_indx=find(Interact(i,:))
        ...
        for tpl_indx = find(Interact(dbl_indx,:))
            ...
            for quad_indx = find(Interact(tpl_indx,:))
                cnt = cnt+1;
                if isempty(find(Interact(quad_indx,:)))
                    index(cnt).index = [i find(Interact(i,:)) dbl_indx find(Interact(dbl_indx,:)) tpl_indx find(Interact(tpl_indx,:))];
                    continue;
                end
                index(cnt).index = [i find(Interact(i,:)) dbl_indx find(Interact(dbl_indx,:)) tpl_indx find(Interact(tpl_indx,:)) quad_indx find(Interact(quad_indx,:))];
            end
        end
    end
end
...
for i=1:length(index)
    ...
    save('i.mat','i')
    save('Num_common_States.mat','Num_common_States')
end
...
%Common_State_Visible(Num_var).Quad_D= rmfield(Common_State_Visible(Num_var).Quad_D,'Visibility_Unambiguity_Score');
Common_State_Visible(Num_var).Quad_DO_cis.Unambiguity_Score = Unambiguity_Score;
Common_State_Visible(Num_var).Quad_DO_cis.Mean_Common_State = Mean_Common_State;
Common_State_Visible(Num_var).Quad_DO_cis.Num_State_Members = Num_State_Members;
Common_State_Visible(Num_var).Quad_DO_cis.index = index;
Common_State_Visible(Num_var).Quad_DO_cis.Mean_Common_in_Visible = Mean_Common_in_Visible;
Common_State_Visible(Num_var).Quad_DO_cis.Sum_P_Detection_in_Visibles = Sum_P_Detection_in_Visibles;
